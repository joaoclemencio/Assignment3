rankhospital <- function(state, outcome, num = "best") {
    ...
    outcome.subset <- outcome.subset[order(
        outcome.subset$Hospital.30.Day.Death..Rate,
        outcome.subset$Hospital.Name,
        na.last = NA
    ),]
    ...
    if(num == "best"){
        ...
        num <- nrow(outcome.subset)
    }
    
    as.character(outcome.subset[num,"Hospital.Name"])
